    ...
    if len(args) > 1:
        ...
        with download_dict_lock:
            ...
            try:
                ...
                pass
    if len(args) == 1:
        msg = f"🤦🏻‍♂️ Please reply to the <code>/{BotCommands.MirrorCommand}</code> message which was used to start the download or send <code>/{BotCommands.CancelMirror} GID</code> to cancel it!"
        if mirror_message and mirror_message.message_id not in keys:
            if BotCommands.MirrorCommand in mirror_message.text or \
               ...
               BotCommands.UnzipMirrorCommand in mirror_message.text:
                msg1 = "🤦🏻‍♂️ Mirror Already Have Been Cancelled"
...
def cancel_all(update, context):
    ...
    while True:
        ...
        if dl:
            ...
            break
    delete_all_messages()
    sendMessage(f'🗄️ {count}\n✅ Download(s) has been Cancelled!', context.bot, update)
