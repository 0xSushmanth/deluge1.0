...
async def init_search(client, message, query, sukebei):
    ...
    if not result:
        ...
        buttons = [InlineKeyboardButton(f'1/{pages}', 'nyaa_nop'), InlineKeyboardButton(f'➡', 'nyaa_next')]
        if pages == 1:
            buttons.pop()
...
async def nyaa_callback(client, callback_query):
    ...
    async with callback_lock:
        ...
        buttons = [InlineKeyboardButton(f'⬅', 'nyaa_back'), InlineKeyboardButton(f'📖{current_page}/{pages}', 'nyaa_nop'), InlineKeyboardButton(f'➡', 'nyaa_next')]
        if ttl_ended:
