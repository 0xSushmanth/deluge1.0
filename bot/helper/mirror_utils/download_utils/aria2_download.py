...
class AriaDownloadHelper(DownloadHelper):
            ...
            if TORRENT_DIRECT_LIMIT is not None:
                ...
                if 'GB' in limit or 'gb' in limit:
                    if size > limitint * 1024**3:
                        dl.getListener().onDownloadError(f'⚠️ Torrent/Direct limit : {TORRENT_DIRECT_LIMIT}.\n📂 Your File/Folder size : {get_readable_file_size(size)}')
                        aria2.remove([download])                        
                        return
    ...
    def __onDownloadError(self, api, gid):
        sleep(1)  # sleep for split second to ensure proper dl gid update from onDownloadComplete
        dl = getDownloadByGid(gid)
        download = api.get_download(gid)
