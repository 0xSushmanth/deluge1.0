...
class GoogleDriveHelper:
    ...
    def getIdFromUrl(link: str):
        if "folders" in link or "file" in link:
            ...
            if res is None:
                raise IndexError("🤷🏻‍♂️ GDrive ID not found.")
            return res.group(5)
    ...
    def deletefile(self, link: str):
        ...
        try:
            ...
            msg = "🗑️ Successfully deleted"
        except HttpError as err:
            LOGGER.error(str(err))
            if "File not found" in str(err):
                msg = "🤷🏻‍♂️ No such file exist"
            else:
                msg = "⚠️ Something went wrong check log"
        finally:
            return msg
    ...
    def clone(self, link):
        ...
        try:
            ...
            if meta.get("mimeType") == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                if INDEX_URL is not None:
                    ...
                    if SHORTENER is not None and SHORTENER_API is not None:
                        ...
                        buttons.buildbutton("📥 DDL", siurl)
                    else:
                        buttons.buildbutton("📥 DDL", url)
            ...
            if "User rate limit exceeded." in str(err):
                msg = "⚠️ User rate limit exceeded."
            elif "File not found" in str(err):
                msg = "🤷🏻‍♂️ File not found."
            else:
                msg = f"❌ Error.\n{err}"
            return msg, ""
    ...
    def count(self, link):
        ...
        try:
            ...
            if drive_file['mimeType'] == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                msg += f'<b>🧾 Filename: </b><code>{meta.get("name")}</code>'
                msg += f'<b>💾 Size: </b><code>{get_readable_file_size(self.transferred_size)}</code>'
                msg += f'\n<b>🗄️ Type: </b><code>Folder</code>'
                msg += f'\n<b>🗃️ SubFolders: </b><code>{self.total_folders}</code>'
                msg += f'\n<b>📁 Files: </b><code>{self.total_files}</code>'
            else:
